import { createTool } from '@mastra/core/tools';
import { z } from 'zod';
import { env } from '../../lib/env';
import { ncaRequest } from '../lib/nca';

export const generateAss = createTool({
  id: 'generateAss',
  description: 'Generate an ASS subtitle file from a public media URL. Transcribes speech and formats it as styled subtitles.',
  inputSchema: z.object({
    mediaUrl: z.string().url(),
    language: z.string().optional(),
    settings: z.record(z.string(), z.any()).optional(),
    webhookUrl: z.string().url().optional(),
    requestId: z.string().optional(),
  }),
  outputSchema: z.object({
    jobId: z.string(),
    status: z.enum(['queued', 'completed']),
    resultUrl: z.string().url().optional(),
    message: z.string(),
  }),
  execute: async ({ mediaUrl, language, settings, webhookUrl, requestId }) => {
    const body = {
      media_url: mediaUrl,
      ...(language && { language }),
      ...(settings && { settings }),
      ...(webhookUrl ?? env.NCA_DEFAULT_WEBHOOK_URL
        ? { webhook_url: webhookUrl ?? env.NCA_DEFAULT_WEBHOOK_URL }
        : {}),
      ...(requestId && { id: requestId }),
    };

    const result = await ncaRequest<{
      code: number;
      job_id: string;
      response: string | null;
      message: string;
    }>('/v1/media/generate/ass', body);

    const isCompleted = typeof result.response === 'string' && result.response.length > 0;

    return {
      jobId: result.job_id,
      status: isCompleted ? 'completed' as const : 'queued' as const,
      resultUrl: isCompleted ? result.response! : undefined,
      message: result.message,
    };
  },
});
